...
async def program() -> None:
    ...
    for i, temp in enumerate(SWEEP_TEMPERATURES):
        ...
        exist2 = os.path.exists(volt_filename)
        if i < len(SWEEP_TEMPERATURES) - 1:
            ...
            # Check if the files already exist
            exist3 = os.path.exists(os.path.join(dir, name_format_temp + "00001.dat"))
        ...
        print(dt.datetime.now(), "Setting He3 heater temperature (via sweep) to", temp, "K...")
        await control.set_parameter_setpoint(parameter=tramea_signals.HE4_EXTRA_TEMP_SENSOR_SETPOINT,
                                             setpoint=temp,
                                             sweep_timeout=5
                                             )
        print(dt.datetime.now(), "Stopped sweep.")
        ...
        if not exist1:
            ...
            val = await control.set_parameter_and_stabilise(
                parameter=tramea_signals.HE4_EXTRA_TEMP_SENSOR,
                setpoint=temp,
                std=0.05,
                time=60*2
            )
        ...
        if not exist2:
            ...
            if i < len(SWEEP_TEMPERATURES) - 1:
                if not exist3:
                    ...
                    # Set the He4 VTI temperature to match the new He3 heater setpoint.
                    print(dt.datetime.now(), "Setting VTI temperature to", SWEEP_TEMPERATURES[i+1], "K...")                                                                                 
                    await control.set_parameter_setpoint(parameter=tramea_signals.HE4_VTI_TEMPERATURE_SETPOINT,
                        setpoint=SWEEP_TEMPERATURES[i+1],
                        approaching_setpoint=temp,
                        sweep_timeout=5
                        )
                    # Wait 10 minutes to stabilise it.
                    print(dt.datetime.now(), "Done. Waiting 10 minutes to stabilise.")    
                    await asyncio.sleep(10*60)
                else:
                    print(dt.datetime.now(), f"Temperature ramp between {temp} and {SWEEP_TEMPERATURES[i+1]} already measured, skipping...")
